...
try:
    ...
    if not attended_df.empty:
        ...
        with col1:
            st.markdown("#### [영등포구 갑] 상세")
            gap_data = attended_df[attended_df['지역구_auto'] == "갑"][['날짜', '시간', '행사명']]
            st.dataframe(gap_data, use_container_width=True, hide_index=True)
        with col2:
            st.markdown("#### [영등포구 을] 상세")
            eul_data = attended_df[attended_df['지역구_auto'] == "을"][['날짜', '시간', '행사명']]
            st.dataframe(eul_data, use_container_width=True, hide_index=True)
    else:
        st.write("참석 데이터가 없어 통계를 표시할 수 없습니다.")
