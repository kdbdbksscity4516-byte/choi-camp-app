    ...
    if not df.empty:
        ...
        for idx, row in filtered_df.iterrows():
            with st.container(border=True):
                ...
                # 2단: 참석/불참 버튼 (좌우 나란히 배치)
                if status == '미체크':
                    col1, col2 = st.columns(2) # 5:5 비율로 나눔
                    if col1.button("🟢 참석", key=f"att_{idx}", use_container_width=True):
                        st.toast("참석 기록 중...") # 실제 연동 시 시트에 기록됨
                    if col2.button("🔴 불참", key=f"no_{idx}", use_container_width=True):
                        st.toast("불참 기록 중...")
                else:
                    # 선택 완료 시: 결과와 수정 버튼을 한 줄에 배치
