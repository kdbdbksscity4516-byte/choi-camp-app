...
# [로직] 주소 기반 동네 분류 함수 (사무장님 지정 기준)
def get_dong_group(address):
    address = str(address)
    # 영등포구 갑 분류
    gap_dongs = ["영등포동", "영등포본동", "당산1동", "당산2동", "도림동", "문래동", "양평1동", "양평2동", "신길1동", "신길2동", "신길3동"]
    # 영등포구 을 분류
    eul_dongs = ["여의동", "신길4동", "신길5동", "신길6동", "신길7동", "대림1동", "대림2동", "대림3동"]
    
    for dong in gap_dongs:
        if dong in address: return "갑", dong
    for dong in eul_dongs:
        if dong in address: return "을", dong
    return "기타", "기타"
...
try:
    ...
    # [하단 분석] 동네별 참석 숫자 집계 표
    st.divider()
    st.subheader("📊 지역구별 누적 참석 통계")
    
    attended_df = df[df['참석여부'].str.strip() == '참석'].copy()
    if not attended_df.empty:
        # 지역구 및 동네 분류 적용
        attended_df[['지역구', '동네']] = attended_df.apply(lambda x: pd.Series(get_dong_group(x['주소'])), axis=1)
        ...
        with col1:
            st.markdown("#### [영등포구 갑]")
            gap_list = ["영등포동", "영등포본동", "당산1·2동", "도림동", "문래동", "양평1·2동", "신길1·2·3동"]
            # 동네 이름 매칭을 위해 실제 데이터와 화면 표시용 동네 그룹화
            gap_res = []
            for d in gap_list:
                count = len(attended_df[(attended_df['지역구'] == "갑") & (attended_df['동네'].str.contains(d[:2]))])
                gap_res.append({"동네": d, "참석 숫자": count})
            st.dataframe(pd.DataFrame(gap_res), use_container_width=True, hide_index=True)

        # 3. 영등포구 을 상세 숫자 표
        with col2:
            st.markdown("#### [영등포구 을]")
            eul_list = ["여의동", "신길4·5·6·7동", "대림1·2·3동"]
            eul_res = []
            for d in eul_list:
                count = len(attended_df[(attended_df['지역구'] == "을") & (attended_df['동네'].str.contains(d[:2]))])
                eul_res.append({"동네": d, "참석 숫자": count})
